...
{	int thread_n;
	...
	if (myRank == MASTER) {
		...
		while (task_row < x_point_n || working_n > 0) {
			...
			if (msg != STANDBY) {
				MPI_Recv(&job_n, 1, MPI_INT, slave_rank, MSG_TAG, MPI_COMM_WORLD, &status);
			...
			if (task_row < x_point_n) {
				msg = task_row;
				job_n = min(thread_n, x_point_n - task_row);
				task_row += job_n;
				working_n++;
			} else
				msg = NO_JOB;
			...
			if (msg != NO_JOB)
				MPI_Send(&job_n, 1, MPI_INT, slave_rank, MSG_TAG, MPI_COMM_WORLD);
		}
	} else {
		msg = STANDBY;
		while (1) {
			...
			if (msg != STANDBY) {
				MPI_Send(&job_n, 1, MPI_INT, MASTER, MSG_TAG, MPI_COMM_WORLD);
				for (int i = 0; i < job_n; ++i)
					MPI_Send(canvas[msg + i], y_point_n, MPI_INT, MASTER, DATA_TAG, MPI_COMM_WORLD);
			}
			...
			MPI_Recv(&job_n, 1, MPI_INT, MASTER, MSG_TAG, MPI_COMM_WORLD, &status);
			load += job_n;

			int i, col;
			#pragma omp parallel private(i, col, repeats, z, c, lengthsq, temp) num_threads(job_n)
			{	
				#pragma omp for schedule (static) nowait 
				for (i = 0; i < job_n; ++i) {
					//printf("Rank %d  start = %d, do %d\n", myRank, task_row, task_row + i);
					for (col = 0; col < y_point_n; ++col) {
						z.real = 0.0;
						z.imag = 0.0;
						c.real = x_left + (double)(task_row + i) * ((x_right - x_left)/(double)x_point_n);
						...
						canvas[task_row + i][col] = 1024*1024*(repeats%256);
					}		
				}
			}
			msg = task_row;
	...
	printf("%d\t%llu.%03llu\t%d\n", myRank, (end_utime - start_utime)/1000, (end_utime - start_utime)%1000, load);
