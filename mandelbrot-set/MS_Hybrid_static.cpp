...
{	int thread_n;
	...
	int v_x_point_n;
	if (x_point_n % nProc != 0)	v_x_point_n = (x_point_n/nProc +1) * nProc;
	else						v_x_point_n = x_point_n;

	canvas = new int*[v_x_point_n];
	for (int r = 0; r < v_x_point_n; ++r)
	...
	int task_row, col;
	int gap = 0;
	gap = v_x_point_n / nProc;
	#pragma omp parallel private(task_row, col, repeats, z, c, lengthsq, temp) num_threads(thread_n)
	{
		#pragma omp for schedule (static) nowait
		for (task_row = gap*myRank; task_row < gap*(myRank +1); ++task_row) {
	...
	if (myRank == MASTER) {
		...
		for (int r = gap * myRank; r < gap*(myRank+1); ++r) {
			MPI_Send(&r, 1, MPI_INT, MASTER, MSG_TAG, MPI_COMM_WORLD);
			MPI_Send(canvas[r], y_point_n, MPI_INT, MASTER, DATA_TAG, MPI_COMM_WORLD);
		}
		msg = NO_RESULT;
