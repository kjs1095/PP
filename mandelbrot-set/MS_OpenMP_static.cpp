 /* 
   ...
   static version, partition by rows
 */
	...
	{	tid = omp_get_thread_num();
		...
		for (i = 0; i < x_point_n; ++i) {
			...
			for (j = 0; j < y_point_n; ++j) {
				z.real = 0.0;
				z.imag = 0.0;
				c.real = x_left + (double)i * ((x_right - x_left)/(double)x_point_n);
				c.imag = y_lower + (double)j * ((y_upper - y_lower)/(double)y_point_n);
				repeats = 0;
				lengthsq = 0.0;
				...
				canvas[i][j] = 1024*1024*(repeats%256);
			}
		}
