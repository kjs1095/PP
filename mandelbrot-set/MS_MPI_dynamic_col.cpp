...
{	int thread_n;
	...
	if (myRank == MASTER) {
		task_col = 0;
		int working_n = 0;
		while (task_col < y_point_n || working_n > 0) {
		...
		while (1) {
			...
			if (msg != STANDBY)
				MPI_Send(canvas[msg], x_point_n, MPI_INT, MASTER, DATA_TAG, MPI_COMM_WORLD);
		
			MPI_Recv(&task_col, 1, MPI_INT, MASTER, MSG_TAG, MPI_COMM_WORLD, &status);
			if (task_col == NO_JOB)	break;
